package chess;

import java.util.ArrayList;
import java.util.Collection;
import java.util.List;

public class PieceMovesCalculator {
    private static ChessBoard board;
    private static ChessPosition myPosition;

    public static Collection<ChessMove> bishop_moves_list(ChessBoard board, ChessPosition myPosition){
        int col = myPosition.getColumn();
        int row = myPosition.getRow();
        Collection<ChessMove> move_list = new ArrayList<>();
        if (col < 8 && row < 8) {
            int col_iter = 1;
            int row_iter = 1;
            while (col + col_iter <= 8 && row + row_iter <=8) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col + col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    col_iter += 1;
                    row_iter +=1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (col > 1 && row > 1) {
            int col_iter = -1;
            int row_iter = -1;
            while (col + col_iter > 0 && row + row_iter > 0) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col + col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    col_iter -= 1;
                    row_iter -=1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (row < 8 && col > 1) {
            int row_iter = 1;
            int col_iter = -1;
            while (row + row_iter <= 8 && col + col_iter >= 1) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col+ col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    row_iter += 1;
                    col_iter -= 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (row > 1 && col < 8) {
            int row_iter = -1;
            int col_iter = 1;
            while (row + row_iter > 0 && col + col_iter <= 8) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col+ col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    row_iter -= 1;
                    col_iter += 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        return move_list;
    }
    public static Collection<ChessMove> king_moves_list(ChessBoard board, ChessPosition myPosition){
        int col = myPosition.getColumn();
        int row = myPosition.getRow();
        Collection<ChessMove> move_list = new ArrayList<>();
        int col_iter = -1;
        while (col_iter < 2)
        {
            int row_iter = -1;
            while (row_iter < 2)
            {
                if(row + row_iter >= 1 && row + row_iter <=8 && col + col_iter >=1 && col + col_iter <=8) {
                    ChessPosition new_position = new ChessPosition(row + row_iter, col + col_iter);
                    if (board.getPiece(new_position) == null) {
                        ChessMove move = new ChessMove(myPosition, new_position, null);
                        move_list.add(move);
                    } else if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                        ChessMove move = new ChessMove(myPosition, new_position, null);
                        move_list.add(move);
                    }
                }
                row_iter = row_iter + 1;
            }
            col_iter +=1;
        }
        return move_list;
    }
    public static Collection<ChessMove> knight_moves_list(ChessBoard board, ChessPosition myPosition){
        PieceMovesCalculator.board = board;
        PieceMovesCalculator.myPosition = myPosition;
        int col = myPosition.getColumn();
        int row = myPosition.getRow();
        Collection<ChessMove> move_list = new ArrayList<>();
        if(myPosition.getColumn() > 2)
        {
            if(myPosition.getRow() < 8)
            {
                ChessPosition new_position = new ChessPosition(row + 1, col - 2);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
        }
        if(myPosition.getColumn() < 7)
        {
            if(myPosition.getRow() < 8)
            {
                ChessPosition new_position = new ChessPosition(row + 1, col + 2);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
        }
        if(myPosition.getColumn() > 2)
        {
            if(myPosition.getRow() > 1)
            {
                ChessPosition new_position = new ChessPosition(row -1, col -2);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
        }
        if(myPosition.getColumn() > 1)
        {
            if(myPosition.getRow() > 2)
            {
                ChessPosition new_position = new ChessPosition(row - 2, col - 1);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
        }
        if(myPosition.getColumn() > 1)
        {
            if(myPosition.getRow() < 7)
            {
                ChessPosition new_position = new ChessPosition(row + 2, col - 1);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
        }
        if(myPosition.getColumn() < 8)
        {
            if(myPosition.getRow() > 2)
            {
                ChessPosition new_position = new ChessPosition(row - 2, col + 1);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
        }
        if(myPosition.getColumn() < 8)
        {
            if(myPosition.getRow() < 7)
            {
                ChessPosition new_position = new ChessPosition(row + 2, col + 1);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
        }
        if(myPosition.getColumn() < 7)
        {
            if(myPosition.getRow() > 1)
            {
                ChessPosition new_position = new ChessPosition(row - 1, col + 2);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
        }
        return move_list;
    }
    public static Collection<ChessMove> pawn_moves_list(ChessBoard board, ChessPosition myPosition){
        Collection<ChessMove> move_list = new ArrayList<>();
        int col = myPosition.getColumn();
        int row = myPosition.getRow();
        if(board.getPiece(myPosition).getTeamColor() == ChessGame.TeamColor.WHITE)
        {
            if(row == 2)
            {
                ChessPosition new_position = new ChessPosition(row + 2, col);
                ChessPosition mid_position = new ChessPosition(row + 1, col);
                if (board.getPiece(new_position) == null && board.getPiece(mid_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
            ChessPosition new_position = new ChessPosition(row + 1, col);
            if (board.getPiece(new_position) == null) {
                if (row != 7) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else {
                    ChessMove move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.KNIGHT);
                    move_list.add(move);
                    move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.QUEEN);
                    move_list.add(move);
                    move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.ROOK);
                    move_list.add(move);
                    move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.BISHOP);
                    move_list.add(move);
                }
            }
            if(col > 1) {
                new_position = new ChessPosition(row + 1, col -1);
                if (board.getPiece(new_position) != null) {
                    if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                        if (row != 7) {
                            ChessMove move = new ChessMove(myPosition, new_position, null);
                            move_list.add(move);
                        }
                        else {
                            ChessMove move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.KNIGHT);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.QUEEN);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.ROOK);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.BISHOP);
                            move_list.add(move);
                        }
                    }
                }
            }
            if(col < 8) {
                new_position = new ChessPosition(row + 1, col + 1);
                if (board.getPiece(new_position) != null) {
                    if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                        if (row != 7) {
                            ChessMove move = new ChessMove(myPosition, new_position, null);
                            move_list.add(move);
                        }
                        else {
                            ChessMove move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.KNIGHT);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.QUEEN);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.ROOK);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.BISHOP);
                            move_list.add(move);
                        }
                    }
                }
            }
        }
        if(board.getPiece(myPosition).getTeamColor() == ChessGame.TeamColor.BLACK)
        {
            if(row == 7)
            {
                ChessPosition new_position = new ChessPosition(row - 2, col);
                ChessPosition mid_position = new ChessPosition(row - 1, col);
                if (board.getPiece(new_position) == null && board.getPiece(mid_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
            }
            ChessPosition new_position = new ChessPosition(row - 1, col);
            if (board.getPiece(new_position) == null) {
                if (row != 2) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                }
                else {
                    ChessMove move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.KNIGHT);
                    move_list.add(move);
                    move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.QUEEN);
                    move_list.add(move);
                    move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.ROOK);
                    move_list.add(move);
                    move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.BISHOP);
                    move_list.add(move);
                }
            }
            if(col > 1) {
                new_position = new ChessPosition(row - 1, col -1);
                if (board.getPiece(new_position) != null) {
                    if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                        if (row != 2) {
                            ChessMove move = new ChessMove(myPosition, new_position, null);
                            move_list.add(move);
                        }
                        else {
                            ChessMove move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.KNIGHT);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.QUEEN);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.ROOK);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.BISHOP);
                            move_list.add(move);
                        }
                    }
                }
            }
            if(col < 8) {
                new_position = new ChessPosition(row - 1, col + 1);
                if (board.getPiece(new_position) != null) {
                    if (board.getPiece(new_position).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                        if (row != 2) {
                            ChessMove move = new ChessMove(myPosition, new_position, null);
                            move_list.add(move);
                        }
                        else {
                            ChessMove move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.KNIGHT);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.QUEEN);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.ROOK);
                            move_list.add(move);
                            move = new ChessMove(myPosition, new_position, ChessPiece.PieceType.BISHOP);
                            move_list.add(move);
                        }
                    }
                }
            }
        }
        return move_list;
    }
    public static Collection<ChessMove> queen_moves_list(ChessBoard board, ChessPosition myPosition){
        Collection<ChessMove> move_list = new ArrayList<>();
        int col = myPosition.getColumn();
        int row = myPosition.getRow();
        if (col < 8) {
            int col_iter = 1;
            while (col + col_iter <= 8) {
                ChessPosition new_position = new ChessPosition(row, col + col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    col_iter += 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (col > 1) {
            int col_iter = -1;
            while (col + col_iter > 0) {
                ChessPosition new_position = new ChessPosition(row, col + col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    col_iter -= 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (row < 8) {
            int row_iter = 1;
            while (row + row_iter <= 8) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    row_iter += 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (row > 1) {
            int row_iter = -1;
            while (row + row_iter > 0) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    row_iter -= 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (col < 8 && row < 8) {
            int col_iter = 1;
            int row_iter = 1;
            while (col + col_iter <= 8 && row + row_iter <=8) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col + col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    col_iter += 1;
                    row_iter +=1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (col > 1 && row > 1) {
            int col_iter = -1;
            int row_iter = -1;
            while (col + col_iter > 0 && row + row_iter > 0) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col + col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    col_iter -= 1;
                    row_iter -=1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (row < 8 && col > 1) {
            int row_iter = 1;
            int col_iter = -1;
            while (row + row_iter <= 8 && col + col_iter >= 1) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col+ col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    row_iter += 1;
                    col_iter -= 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (row > 1 && col < 8) {
            int row_iter = -1;
            int col_iter = 1;
            while (row + row_iter > 0 && col + col_iter <= 8) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col+ col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    row_iter -= 1;
                    col_iter += 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        return move_list;
    }
    public static Collection<ChessMove> rook_moves_list(ChessBoard board, ChessPosition myPosition){
        int col = myPosition.getColumn();
        int row = myPosition.getRow();
        Collection<ChessMove> move_list = new ArrayList<>();
        if (col < 8) {
            int col_iter = 1;
            while (col + col_iter <= 8) {
                ChessPosition new_position = new ChessPosition(row, col + col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    col_iter += 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (col > 1) {
            int col_iter = -1;
            while (col + col_iter > 0) {
                ChessPosition new_position = new ChessPosition(row, col + col_iter);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    col_iter -= 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (row < 8) {
            int row_iter = 1;
            while (row + row_iter <= 8) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    row_iter += 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        if (row > 1) {
            int row_iter = -1;
            while (row + row_iter > 0) {
                ChessPosition new_position = new ChessPosition(row + row_iter, col);
                if (board.getPiece(new_position) == null) {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    row_iter -= 1;
                } else if (board.getPiece(new_position).getTeamColor() == board.getPiece(myPosition).getTeamColor()) {
                    break;
                } else {
                    ChessMove move = new ChessMove(myPosition, new_position, null);
                    move_list.add(move);
                    break;
                }
            }
        }
        return move_list;
    }
}
